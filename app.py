...
def _find_step8_module_file() -> Path | None:
    """Επιστρέφει κατά προτεραιότητα την έκδοση Βήματος 8 με προαιρετικό φίλο."""
    for candidate in STEP8_MODULE_CANDIDATES:
        if candidate.exists():
            return candidate
    return None
...
if _find_step8_module_file() is None:
    missing.append(
        "Δεν βρέθηκε ούτε step8_fixed_final_OPTIONAL_FRIEND.py ούτε step8_fixed_final.py"
    )
if missing:
                ...
                if not sheet_names:
                    ...
                    if not candidates:
                        ...
                        if total_scenario_cols == 0:
                            ...
                            st.error(
                                "❌ Δεν βρέθηκε πλήρης και έγκυρη κατανομή, επειδή δεν "
                                "τοποθετήθηκαν όλοι οι μαθητές. Δεν δημιουργήθηκε τελικό "
                                "αποτέλεσμα και δεν διατηρήθηκε κανένα αποτέλεσμα "
                                "προηγούμενης εκτέλεσης."
                            )
                        else:
                            st.error(
                                "❌ Δεν βρέθηκε έγκυρη κατανομή. Δεν δημιουργήθηκε τελικό "
                                "αποτέλεσμα και δεν διατηρήθηκε κανένα αποτέλεσμα "
                                "προηγούμενης εκτέλεσης."
                            )
                    else:
...
if st.session_state.get("last_final_path"):
    ...
    st.write("Εφαρμογή **Βήματος 8**: δύο κύκλοι παιδαγωγικής εξισορρόπησης και τρίτος κύκλος κοινωνικής αποκατάστασης μέσω του προαιρετικού φίλου.")
    ...
    if st.button("🎯 Βέλτιστη Κατανομή", type="primary", use_container_width=True, key="run_step8"):
        try:
            ...
            if not input_source or not Path(input_source).exists():
                ...
                # Load step8 module — προτιμάται η έκδοση με ΠΡΟΑΙΡΕΤΙΚΟ_ΦΙΛΟ.
                step8_module_path = _find_step8_module_file()
                if step8_module_path is None:
                    raise FileNotFoundError(
                        "Δεν βρέθηκε αρχείο Βήματος 8. Ανέβασε το "
                        "step8_fixed_final_OPTIONAL_FRIEND.py ή το step8_fixed_final.py."
                    )
                s8 = _load_module(step8_module_path.stem, step8_module_path)
                st.caption(f"🧩 Χρησιμοποιείται: {step8_module_path.name}")
                ...
                with st.spinner("🎯 Phase 2/2: Optimizing..."):
                    ...
                    cycle1_count = sum(1 for s in swaps if s.get("cycle") == 1)
                    cycle2_count = sum(1 for s in swaps if s.get("cycle") == 2)
                    cycle3_count = sum(1 for s in swaps if s.get("cycle") == 3)
                    st.write(
                        f"**Κινήσεις ανά κύκλο:** Κύκλος 1: {cycle1_count} · "
                        f"Κύκλος 2: {cycle2_count} · Κύκλος 3: {cycle3_count}"
                    )
                    if cycle3_count:
                        with st.expander("🤝 Αποκαταστάσεις μέσω προαιρετικού φίλου", expanded=True):
                            rows = []
                            for swap in swaps:
                                if swap.get("cycle") != 3:
                                    continue
                    ...
                    st.success(f"🎉 Ολοκληρώθηκε! Εφαρμόστηκαν συνολικά {len(swaps)} νόμιμες κινήσεις/ανταλλαγές, από τις οποίες {cycle3_count} αφορούσαν τον προαιρετικό φίλο.")
